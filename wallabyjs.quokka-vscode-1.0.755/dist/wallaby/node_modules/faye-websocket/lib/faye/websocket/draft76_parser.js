...
Draft76Parser.prototype.handshakeResponse = function(head) {
  ...
  var response = Buffer.from('HTTP/1.1 101 Web Socket Protocol Handshake\r\n' +
    'Upgrade: WebSocket\r\n' +
    'Connection: Upgrade\r\n' +
  ...
  if (signature) {
    tmp = Buffer.alloc(response.length + signature.length);
    response.copy(tmp, 0);
...
Draft76Parser.prototype.handshakeSignature = function(head) {
  ...
  MD5.update(bigEndian(value1), 'binary');
  MD5.update(bigEndian(value2), 'binary');
  MD5.update(head.toString('binary'), 'binary');
...
Draft76Parser.prototype.close = function(code, reason, callback, context) {
  ...
  if (this._closing) this._socket.send(Buffer.from([0xFF, 0x00]));
  this._closed = true;
